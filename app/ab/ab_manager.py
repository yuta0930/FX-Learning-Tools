...
class ABManager:
    ...
    def evaluate_and_promote(self) -> Tuple[str, Dict]:
        ...
        m = df.groupby("variant").apply(metrics)
        if min_arm > 0:
            for arm in ("A", "B"):
                if arm not in m.index or m.loc[arm, "N"] < min_arm:
                    return "none", m.to_dict()
        if not set(["A","B"]).issubset(m.index):
            return "none", m.to_dict()
        # simple compare with effect size threshold
        ...
        if promote_on_sig and diff >= eff:
            # shift allocation 10% towards winner
            ...
            # cooldown: check last promotion timestamp
            ...
            try:
                if cooldown_h > 0 and os.path.exists(self._state_path):
                    ...
                    if (now - last) < (cooldown_h * 3600.0):
                        return winner, m.to_dict()  # respect cooldown
            except Exception:
                pass
            if winner == "A":
