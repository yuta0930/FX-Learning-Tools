...
def test_atr_spike():
    ...
    for v in [10,10,11,9,10]:
        g.record_atr(v)
    allowed, reason = g.allow_new_trade(ts, session='Tokyo', current_atr=20)
    assert not allowed and reason == 'atr_spike_halt'

def test_state_contains_limits():
    cfg = RiskConfig()
    g = TradeGuard(cfg)
    st = g.state()
    assert {'max_day_trades','max_session_trades','loss_cooldown_after'} <= st.keys()
